...
public class Game implements GameInterface {
	...
	private Player [] player;
        static final String [][] registered = {
            {"Pepe", "pepe123", "Pepote"},
            {"Juan", "juan123", "Juanote"},
            {"Luis", "luis123", "Luisote"},
            {"Angel", "angel123", "Angelote"},
            {"Miguel", "miguel123", "Miguelote"},
            {"Paco", "paco123", "Pacote"}
        };
	...
	public void desconectar(byte nplayer) throws RemoteException {
        ...
        public void miMovimiento(byte picked, String playerID) throws RemoteException {
            ...
            for (int i=0; i<player.length;i++) {
                if (player[i].getNickname().equals(playerID)) {
                    player[i].setElection(picked);
                    break;
                }
            }
    ...
    private boolean completeGame() throws RemoteException {
        ...
        boolean complete = true;
            for (int i=0; i<player.length;i++) {
                if (player[i].getElection() == -1) complete = false;
            }
        if (complete) {
            for (int i=0; i<player.length;i++) {
                player[i].muestraJugada(player[0].getElection(), player[1].getElection(), player[2].getElection());
            }
